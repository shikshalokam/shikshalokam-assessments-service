    ...
    static getFilePublicBaseUrl() {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if(cloudStorage == "") {
                    throw new Error(apiResponses.MISSING_CLOUD_STORAGE_PROVIDER);
                }

                if(cloudStorage != "GC" && cloudStorage != "AWS") {
                    throw new Error(apiResponses.INVALID_CLOUD_STORAGE_PROVIDER);
                }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if(signedUrls.length == fileNames.length) {
                    ...
                    return resolve({
                        success : false,
                        message : apiResponses.FAILED_PRE_SIGNED_URL,
                        files : signedUrls
                    });
                }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if(signedUrl[0] && signedUrl[0] != "") {
                    return resolve({
                        success : true,
                        message : apiResponses.URL_GENERATED+"Signed.",
                        url : signedUrl[0],
                        name : gcpFile.name
                    });
                } else {
                    return resolve({
                        success : false,
                        message : apiResponses.FAILED_SIGNED_URL,
                        response : signedUrl
                    });
                }
