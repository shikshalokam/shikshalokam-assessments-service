    ...
    static findSubmissionByEntityProgram(document, requestObject) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                return resolve({
                    message: apiResponses.SUBMISSION_FOUND,
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (submissionDocument.evidencesStatus && submissionDocument.status !== "blocked") {
                    ...
                    for (let iterator = 0; iterator < evidencesArray.length; iterator++) {
                        if (!evidencesArray[iterator].isSubmitted || evidencesArray[iterator].hasConflicts === true) {
                            ...
                            // if(!(evidencesArray[iterator].isActive === false)) {
                                ...
                                result.responseMessage = apiResponses.ALL_ECM_NOT_COMPLETED;
                                break;
                            //}
                        }
                    }
                    ...
                    result.ratingsEnabled = false;
                    result.responseMessage = apiResponses.ASSESSMENT_BLOCK_FOR_RATING;
                }
    ...
    static createEvidencesInSubmission(req, modelName, isSubmission) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                req.body = req.body || {};
                let message = apiResponses.SUBMISSION_COMPLETED;
                ...
                if (req.body.evidence) {
                    ...
                    if (submissionDocument.evidences[req.body.evidence.externalId].isSubmitted === false) {
                        ...
                        message = apiResponses.DUPLICATE_ECM_SUBMISSION;
                    }
                    ...
                    if (allSubmittedEvidence) {
                        ...
                        if (sourceApiHelp == "singleRateApi") {
                            return resolve({
                                ...
                                message: apiResponses.ALL_ECM_NOT_SUBMITTED
                            });
                        }
    ...
    static pushCompletedSubmissionForReporting(submissionId) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (submissionId == "") {
                    throw apiResponses.SUBMISSION_ID_NOT_FOUND;
                }
                ...
                if (!submissionsDocument) {
                    throw apiResponses.SUBMISSION_NOT_FOUND + "or" +SUBMISSION_STATUS_NOT_COMPLETE;
                }
    ...
    static rateSubmissionById(submissionId = "") {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (submissionId == "") {
                    throw new Error(apiResponses.SUBMISSION_ID_NOT_FOUND);
                }
                ...
                if(resultingArray.result.runUpdateQuery) {
                    ...
                    emailClient.pushMailToEmailService(emailRecipients,apiResponses.SUBMISSION_AUTO_RATING_SUCCESS+submissionId,JSON.stringify(resultingArray));
                    return resolve(apiResponses.SUBMISSION_RATING_COMPLETED);
                } else {
                    emailClient.pushMailToEmailService(emailRecipients,apiResponses.SUBMISSION_AUTO_RATING_FAILED+submissionId,JSON.stringify(resultingArray));
                    return resolve(apiResponses.SUBMISSION_RATING_COMPLETED);
                }

            } catch (error) {
                emailClient.pushMailToEmailService(emailRecipients,apiResponses.SUBMISSION_AUTO_RATING_FAILED+submissionId,error.message);
                return reject(error);
            }
        })
    }
